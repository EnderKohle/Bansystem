...
public class BanSystem extends Plugin {
	...
	public void onEnable() {
		...
		ProxyServer.getInstance().getConsole()
				.sendMessage(new TextComponent(BanSystem.PREFIX + "§7Das BanSystem wurde gestartet."));
		...
		if (config.getString("VPN.serverIP").equals("00.00.00.00") && config.getBoolean("VPN.autoban.enable"))
			ProxyServer.getInstance().getConsole().sendMessage(
					new TextComponent(BanSystem.PREFIX + "§cBitte trage die IP des Servers in der config.yml ein."));
	...
	public void onDisable() {
		...
		ProxyServer.getInstance().getConsole()
				.sendMessage(new TextComponent(BanSystem.PREFIX + "§7Das BanSystem wurde gestoppt."));
	...
	private void createConfig() {
		try {
			...
			if (!messagesfile.exists()) {
				...
				messages.set("autoban.words.notify", Arrays.asList(new String[] {
						"%P%§8§m------------------------------",
				...
				messages.set("autoban.ads.notify", Arrays.asList(new String[] {
						"%P%§8§m------------------------------",
						"%P%§e%player% wurde §7für sein Chatverhalten", 
						"%P%§7automatisch §egebannt/gemuted§7.",
						"%P%§7Grund §8» §e%reason%",
			...
			if(!blacklistfile.exists()) {
				...
				blacklist.set("Ads", Arrays.asList(new String[] {".de", ".net",".at",".com",".be",".eu",".shop"}));
